...
export function diffContentView(before: ContentView, after: ContentView): ContentDiff {
  ...
  for (const name of beforeByName.keys()) {
    ...
    const sigB = stable({ layers: b.layers, variables: b.variables, groups: b.groups, events: b.events });
    const sigA = stable({ layers: a.layers, variables: a.variables, groups: a.groups, events: a.events });
    if (sigB !== sigA) {
  ...
  for (const o of before.globalObjects) bo.set(`global/${o.name}`, stable(o));
  for (const o of after.globalObjects) ao.set(`global/${o.name}`, stable(o));
  for (const sc of before.scenes) for (const o of sc.objects) bo.set(`scene/${sc.name}/${o.name}`, stable(o));
  for (const sc of after.scenes) for (const o of sc.objects) ao.set(`scene/${sc.name}/${o.name}`, stable(o));
  const objects = diffKeys(bo, ao);
  const bi = new Map<string, string>();
  const ai = new Map<string, string>();
  for (const sc of before.scenes) for (const i of sc.instances) bi.set(`scene/${sc.name}/${i.id}`, stable(i));
  for (const sc of after.scenes) for (const i of sc.instances) ai.set(`scene/${sc.name}/${i.id}`, stable(i));
  ...
  for (const g of before.globalGroups) bg.set(`global/${g.name}`, stable(g.objects));
  for (const g of after.globalGroups) ag.set(`global/${g.name}`, stable(g.objects));
  for (const sc of before.scenes) for (const g of sc.groups) bg.set(`scene/${sc.name}/${g.name}`, stable(g.objects));
  for (const sc of after.scenes) for (const g of sc.groups) ag.set(`scene/${sc.name}/${g.name}`, stable(g.objects));
  const groups = diffKeys(bg, ag);
  const resources = diffKeys(
    new Map(before.resources.map((r) => [r.name, stable(r)])),
    new Map(after.resources.map((r) => [r.name, stable(r)])),
  );
  const be = new Map<string, string>();
  const ae = new Map<string, string>();
  const walkB = (prefix: string, nodes: ContentView['scenes'][number]['events']): void => {
    nodes.forEach((e, idx) => {
      be.set(`${prefix}/${idx}:${e.id || e.kind}`, stable(e));
      if (e.events.length > 0) walkB(`${prefix}/${idx}:${e.id || e.kind}`, e.events);
    });
  };
  const walkA = (prefix: string, nodes: ContentView['scenes'][number]['events']): void => {
    nodes.forEach((e, idx) => {
      ae.set(`${prefix}/${idx}:${e.id || e.kind}`, stable(e));
      if (e.events.length > 0) walkA(`${prefix}/${idx}:${e.id || e.kind}`, e.events);
    });
  };
  for (const sc of before.scenes) walkB(`scene/${sc.name}`, sc.events);
  for (const sc of after.scenes) walkA(`scene/${sc.name}`, sc.events);
